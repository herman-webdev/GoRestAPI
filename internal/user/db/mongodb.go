...
func (d *db) Create(ctx context.Context, user user.User) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to create a user due to error: %v", err)
	}
...
func (d *db) FindOne(ctx context.Context, id string) (u user.User, err error) {
	...
	if result.Err() != nil {
		...
		return u, fmt.Errorf("failed to find a user by id: %s due to error: %v", id, err)
	}

	if err = result.Decode(&u); err != nil {
		return u, fmt.Errorf("failed to decode a user (id:%s) from DB dut to error: %s", id, err)
	}
